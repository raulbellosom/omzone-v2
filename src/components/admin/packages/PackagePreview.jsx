  ...
  return (
    <Card className="p-5 space-y-5 border-sage/30">
      ...
      <div className="flex flex-wrap items-center gap-4 text-sm text-charcoal-muted">
        ...
        {pkg.capacity && <span>Hasta {pkg.capacity} personas</span>}
      </div>
      ...
      {items.length > 0 && (
        <div className="space-y-2">
          ...
          <ul className="space-y-2">
            {items.map((item, i) => (
              <li key={item.$id || item._tempId || i} className="flex items-start gap-2.5">
                <ItemTypeBadge type={item.itemType} className="mt-0.5 shrink-0" />
